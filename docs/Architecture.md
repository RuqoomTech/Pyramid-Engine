...
Every render pass may bind its own framebuffer and viewport. `RenderSystem` therefore rebinds the default surface and restores the main render extent after each pass; final direct overlays establish their own physical surface viewport again. This prevents fixed-resolution shadow maps and other off-screen targets from corrupting subsequent world framing or UI clipping. All of these binds go through the backend-neutral `IGraphicsDevice::BindFramebuffer(IFramebuffer*)`; `BindFramebufferHandle(u32)` remains only as the device-internal workhorse behind it, and no pass or system call site holds a raw backend handle.
...
`ITexture2D::CreateDepthTarget(width, height, format)` creates a real sampled depth texture through `OpenGLTexture2D` for the mapped depth set (`Depth16`, `Depth24`, `Depth32F`, `Depth24Stencil8`, `Depth32FStencil8`). Creation is transactional: the extent is validated before anything is allocated, and any failure deletes the texture object, clears the handle, and returns `nullptr` with a diagnostic naming the dimensions, so a rejected request leaves nothing half-made. Non-depth formats are rejected with a pointer at framebuffer attachments rather than silently becoming color textures. Depth targets are never mipmapped, and `GL_TEXTURE_COMPARE_MODE` is pinned to `GL_NONE` explicitly at creation, so depth is sampled as an ordinary texture; shadow comparison is a sampler choice an owner opts into, never something creation turns on by surprise.

Sampled depth texture and depth attachment are different facilities that coexist. A **sampled depth texture** is a standalone `ITexture2D` used for reading depth in a shader, created through `CreateDepthTarget`. A **depth attachment** is owned by `OpenGLFramebuffer` as part of a render target: it is the storage a pass writes depth into while rendering, and it is the path a framebuffer with color attachments must use. Use `CreateDepthTarget` when something only needs to read depth; use `OpenGLFramebuffer` attachments when a target needs to be rendered into. Ownership of a shared sampled depth texture still goes through `ResourceRegistry::Textures()` like any other sampled asset, never by mutating a cached `TextureResource` in place.

`OpenGLFramebuffer` owns one framebuffer and its attachments through a single cleanup path. Resizing creates and validates a replacement first, then swaps it into service; failed replacement creation leaves the previous framebuffer intact. `Renderer::RenderTarget` delegates to this same implementation rather than maintaining a second raw OpenGL lifecycle, and binds and unbinds through the neutral device method. `RenderSystem::Resize()` propagates window dimensions to managed targets and passes, while fixed-resolution shadow maps remain unchanged.
