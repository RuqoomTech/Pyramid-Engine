...
The octree supports insertion, removal, incremental synchronization, explicit rebuilding, atomic configuration, bounds-aware nearest-neighbor queries, and public spatial helpers. Nearest traversal visits children by the minimum point-to-node distance and prunes branches once their lower bound exceeds the farthest retained candidate. It tracks the last world AABB for each object. During the normal `SceneManager::Update()` path, new and removed scene objects are reconciled and only objects with changed bounds are removed and reinserted; stable objects do not mutate the tree. Configuration changes validate the complete root bounds/depth/capacity request, build a replacement tree off to the side, reinsert all tracked objects, and only then swap live state. Invalid settings or replacement-construction failures preserve the previous tree. Local bounds default to a unit cube and must currently be assigned by asset code; imported meshes do not yet derive them automatically. Occlusion culling remains a placeholder and is disabled by default.
